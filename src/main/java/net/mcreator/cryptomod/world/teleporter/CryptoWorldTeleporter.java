...
public class CryptoWorldTeleporter implements ITeleporter {
	...
	public static void registerPointOfInterest(RegistryEvent.Register<PoiType> event) {
		poi = new PoiType("crypto_world_portal",
				Sets.newHashSet(ImmutableSet.copyOf(CryptoModBlocks.CRYPTO_WORLD_PORTAL.getStateDefinition().getPossibleStates())), 0, 1)
				...
				for (int j3 = 0; j3 < 2; ++j3) {
					for (int k3 = -1; k3 < 3; ++k3) {
						BlockState blockstate1 = k3 < 0
								? CryptoModBlocks.CRYPTO_WORLD_PORTAL_FRAME.defaultBlockState()
		...
		for (int l1 = -1; l1 < 3; ++l1) {
			for (int j2 = -1; j2 < 4; ++j2) {
				if (l1 == -1 || l1 == 2 || j2 == -1 || j2 == 3) {
					...
					this.level.setBlock(blockpos$mutableblockpos, CryptoModBlocks.CRYPTO_WORLD_PORTAL_FRAME.defaultBlockState(), 3);
				}
			}
		}
		BlockState blockstate = CryptoModBlocks.CRYPTO_WORLD_PORTAL.defaultBlockState().setValue(NetherPortalBlock.AXIS, p_77668_);
