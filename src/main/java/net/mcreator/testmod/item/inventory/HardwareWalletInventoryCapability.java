...
public class HardwareWalletInventoryCapability implements ICapabilitySerializable<CompoundTag> {
	@SubscribeEvent
	@OnlyIn(Dist.CLIENT)
	public static void onItemDropped(ItemTossEvent event) {
		if (event.getEntityItem().getItem().getItem() == TestModModItems.HARDWARE_WALLET) {
	...
	private ItemStackHandler createItemHandler() {
		return new ItemStackHandler(18) {
			...
			public boolean isItemValid(int slot, @Nonnull ItemStack stack) {
				return stack.getItem() != TestModModItems.HARDWARE_WALLET;
			}
